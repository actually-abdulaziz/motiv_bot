...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "get_random":
        await send_random_post(update, context, via_callback=True)

async def send_random_post(update: Update, context: ContextTypes.DEFAULT_TYPE, via_callback=False):
    message_id = get_random_message_id()
    if message_id:
        chat_id = update.effective_chat.id if not via_callback else update.callback_query.message.chat.id
        await context.bot.forward_message(chat_id=chat_id,
                                          from_chat_id=CHANNEL_ID,
                                          message_id=message_id)
    else:
        await update.effective_message.reply_text("Нет сохранённых постов.")
